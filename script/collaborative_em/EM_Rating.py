...
locationUsersFile = pathlib.Path(
    r'D:/Game-Recommendation-System/data/raw_data/steam_users_purchase_play.csv')
steam_clean = pd.read_csv(locationUsersFile, header=1, names=[
                          'user', 'game', 'hrs', 'purchase', 'play'])

locationUsersFile_train = pathlib.Path(
    r'D:/Game-Recommendation-System/data/model_data/steam_user_train.csv')
steam_traind = pd.read_csv(locationUsersFile_train, header=1, names=[
                           'user', 'game', 'hrs', 'purchase', 'play'])

locationUsersFile_test = pathlib.Path(
    r'D:/Game-Recommendation-System/data/model_data/steam_user_test.csv')
steam_test = pd.read_csv(locationUsersFile_test, header=1, names=[
                         'user', 'game', 'hrs', 'purchase', 'play'])

game_freq = steam_traind.groupby(by='game').agg(
    {'user': 'count', 'hrs': 'sum'}).reset_index()
...
steam_traind['game1'] = steam_traind['game'].apply(
    lambda x: re.sub('[^a-zA-Z0-9]', '', x))
steam_clean['game1'] = steam_clean['game'].apply(
    lambda x: re.sub('[^a-zA-Z0-9]', '', x))
# steam_clean.head()
...
def game_hrs_density(GAME, nclass, print_vals=True):
    ...
    game_data = steam_clean[(steam_clean['game1'] == GAME)
                            & (steam_clean['hrs'] > 2)]
    # Log hrs
    game_data['loghrs'] = np.log(steam_clean['hrs'])
    # Calculate the mu,sigma to process Gaussian function
    mu_init = np.linspace(min(game_data['loghrs']), max(
        game_data['loghrs']), nclass).reshape(-1, 1)
    sigma_init = np.array([1] * nclass).reshape(-1, 1, 1)
    gaussian = GaussianMixture(n_components=nclass, means_init=mu_init, precisions_init=sigma_init).fit(
        game_data['loghrs'].values.reshape([-1, 1]))
    # print Gaussian Lambda Mean Sigma
    if print_vals:
        print(' lambda: {}\n mean: {}\n sigma: {}\n'.format(
            gaussian.weights_, gaussian.means_, gaussian.covariances_))
    # Random Generate
    x = np.linspace(min(game_data['loghrs']), max(game_data['loghrs']), 1000)
    # Plot
    dens = pd.DataFrame({'x': x})
    for i in range(nclass):
        dens['y{}'.format(i+1)] = gaussian.weights_[i] * scipy.stats.norm(
            gaussian.means_[i][0], gaussian.covariances_[i][0][0]).pdf(x)
...
game_freq['game1'] = game_freq['game'].apply(
    lambda x: re.sub('[^a-zA-Z0-9]', '', x))
...
steam_clean_pos_idx = steam_clean_pos['game1'].apply(
    lambda x: x in game_users['game1'].values)
steam_clean_pos = steam_clean_pos[steam_clean_pos_idx]
steam_clean_pos['loghrs'] = np.log(steam_clean_pos['hrs'])


# make matrix
games = pd.DataFrame({'game1': sorted(steam_clean_pos['game1'].unique(
)), 'game_id': range(len(steam_clean_pos['game1'].unique()))})
users = pd.DataFrame({'user': sorted(steam_clean_pos['user'].unique(
)), 'user_id': range(len(steam_clean_pos['user'].unique()))})
...
# test dataset user
users_test = pd.DataFrame({'user': sorted(steam_test['user'].unique(
)), 'user_id': range(len(steam_test['user'].unique()))})
# print(users_test)
...
steam_train_idx = steam_train['game1'].apply(
    lambda x: x in game_users['game1'].values)
...
games_train = pd.DataFrame({'game1': sorted(steam_train['game1'].unique(
)), 'game_id': range(len(steam_train['game1'].unique()))})
# List the users in train dataset use for recommend
users_train = pd.DataFrame({'user': sorted(steam_train['user'].unique(
)), 'user_id': range(len(steam_train['user'].unique()))})
...
path1 = pd.DataFrame({'itr': range(1, N+2), 'fobj': fobj, 'fobjp': fobj /
                     max(fobj), 'rmse': rmsej, 'rmsep': rmsej/max(rmsej)})
...
def game_hrs_density_p(pred, GAME=None, nclass=1, print_vals=True):
    ...
    # EM algorithm
    mu_init = np.linspace(min(game_data), max(
        game_data), nclass).reshape(-1, 1)
    sigma_init = np.array([1] * nclass).reshape(-1, 1, 1)
    gaussian = GaussianMixture(n_components=nclass, means_init=mu_init,
                               precisions_init=sigma_init).fit(game_data.values.reshape([-1, 1]))
    if print_vals:
        print(' lambda: {}\n mean: {}\n sigma: {}\n'.format(
            gaussian.weights_, gaussian.means_, gaussian.covariances_))
    # building data frame for plotting
...
reverse_game_dict = {games.iloc[i, 1]: games.iloc[i, 0]
